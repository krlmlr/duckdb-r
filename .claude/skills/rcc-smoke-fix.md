...
`clang-format` is preinstalled. `air` is from
[posit-dev/air](https://github.com/posit-dev/air); install it on demand
(release tarball or `cargo install air`) if `air.toml` is present and the
binary is missing — same fallback policy as the dependency-install note
in Step 4b.

Re-running `R CMD INSTALL` after formatting is unnecessary unless the
formatter actually rewrote one of your edited files.

### 4f. Fold the fix into the failing commit (amend, do not add)
