...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/RtmpLjUIN1/file52e139f48883.csv') AS q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpLjUIN1/file52e139f48883.csv"
...
#>   read_csv_auto('/tmp/RtmpLjUIN1/file52e139f48883.csv')) AS q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpLjUIN1/file52e139f48883.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/RtmpLjUIN1/file52e139f48883.csv')) AS...
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%
