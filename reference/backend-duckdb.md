...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/Rtmpk4yYM8/file594660be123c.csv') AS q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/Rtmpk4yYM8/file594660be123c.csv"
...
#>   read_csv_auto('/tmp/Rtmpk4yYM8/file594660be123c.csv')) AS q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/Rtmpk4yYM8/file594660be123c.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/Rtmpk4yYM8/file594660be123c.csv')) AS...
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%
