...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/RtmpIkTHxJ/file55bc7db0090c.csv') AS q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpIkTHxJ/file55bc7db0090c.csv"
...
#>   read_csv_auto('/tmp/RtmpIkTHxJ/file55bc7db0090c.csv')) AS q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpIkTHxJ/file55bc7db0090c.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/RtmpIkTHxJ/file55bc7db0090c.csv')) AS...
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%
