...
static void BaseRAddFunctionInteger(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	BinaryExecutor::ExecuteWithNulls<int32_t, int32_t, int32_t>(lefts, rights, result, args.size(),
...
static void BaseREqFunctionDouble(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	auto &rights = args.data[1];
	D_ASSERT(rights.GetType() == LogicalType::DOUBLE);
	BinaryExecutor::ExecuteWithNulls<double, double, bool>(lefts, rights, result, args.size(),
                                                           [&](double left, double right, ValidityMask& mask, idx_t idx) {
...
[[cpp11::register]] void rapi_rel_register_functions(duckdb::conn_eptr_t con) {
	...
	base_r_add.AddFunction(ScalarFunction({LogicalType::INTEGER, LogicalType::INTEGER}, LogicalType::INTEGER, BaseRAddFunctionInteger));
	base_r_add.AddFunction(ScalarFunction({LogicalType::DOUBLE, LogicalType::DOUBLE}, LogicalType::DOUBLE, BaseRAddFunctionDouble));

	ScalarFunctionSet base_r_eq("___base_r_eq");
	base_r_eq.AddFunction(ScalarFunction({LogicalType::INTEGER, LogicalType::INTEGER}, LogicalType::BOOLEAN, BaseREqFunctionInteger));
	base_r_eq.AddFunction(ScalarFunction({LogicalType::DOUBLE, LogicalType::DOUBLE}, LogicalType::BOOLEAN, BaseREqFunctionDouble));
	base_r_eq.AddFunction(ScalarFunction({LogicalType::VARCHAR, LogicalType::VARCHAR}, LogicalType::BOOLEAN, BaseREqFunctionString));
